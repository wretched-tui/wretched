  ...
  this.input.on(
    ...
    (this.input._keypressHandler = function (ch, key) {
      ...
      for (const program of Program.instances) {
        if (program.input !== self.input) continue
  ...
  switch (param) {
    ...
    default:
      if (this.truecolor && param[0] === '#') {
        let [_0, rgb, _1, fgbg] = /^(#[0-9a-f]{3,6})(\(\d+\))? (fg|bg)/i.exec(
          param,
        )
      ...
      // 256-color fg and bg
      if (param[0] === '#') {
        param = param.replace(/#(?:[0-9a-f]{3}){1,2}(\(\d+\))?/i, c =>
          colors.match(c),
        )
      }
...
Program.prototype.fg = Program.prototype.setForeground = function (
  color,
  enabled,
) {
...
Program.prototype.bg = Program.prototype.setBackground = function (
  color,
  enabled,
) {
