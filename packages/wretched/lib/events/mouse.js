...
function isMousePressStart(event) {
    return (event.name.startsWith('mouse.button.') &&
        ['down', 'enter'].some(function (suffix) { return event.name.endsWith(suffix); }));
}
/**
...
function isMousePressExit(event) {
    return (event.name.startsWith('mouse.button.') &&
        ['up', 'exit', 'cancel'].some(function (suffix) { return event.name.endsWith(suffix); }));
}
function isMousePressEnd(event) {
    return (event.name.startsWith('mouse.button.') &&
        ['up', 'cancel'].some(function (suffix) { return event.name.endsWith(suffix); }));
}
/**
...
function isMouseDragging(event) {
    return (event.name.startsWith('mouse.button.') &&
        ['.up', '.cancel', '.enter', '.exit'].every(function (suffix) { return !event.name.endsWith(suffix); }));
}
